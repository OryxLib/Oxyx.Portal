...
namespace Oryx.CartoonPublisher
{
    public class UplaodCartoonManual
    {
        ...
        public async Task BuildChapter(CommonDbContext dbContext, string categoryName)
        {
            ...
            {
                var streamStreamList = await PDFTool.PDFToImage(pdfItem);
                //chapter level
                ...
                foreach (var imgStream in streamStreamList)
                {
                    ...
                    await prepareImage(dbContext, chapterContent, imgStream, imgName, imgIndex);
                    imgIndex += 1;
                }
                chapterIndex++;
            }
        ...
        private async Task prepareImage(CommonDbContext dbContext, ContentEntry chapterContent, Stream imgStream, string imgName, int imgIndex)
        {
            ...
            if (mediaImg == null)
            {
                ...
                await QiniuTool.UploadImage(imgStream, imgName);
                chapterContent.MediaResource.Add(new FileEntry
                {
                    ...
                    Id = Guid.NewGuid(),
                    Order = imgIndex,
                    Tag = chapterContent.Id.ToString()
                });
                await dbContext.SaveChangesAsync();
            }
        }

        private async Task<ContentEntry> prepareChapter(CommonDbContext dbContext, Categories category, string chapterName, int chapterIndex)
        {
            ...
            if (chapter == null)
            {
                ...
                await dbContext.ContentEntry.AddAsync(chapter);
                await dbContext.SaveChangesAsync();
