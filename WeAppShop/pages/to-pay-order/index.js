...
Page({
  ...
  createOrder: function (e) {
    ...
    var postData = {
      GoodsList: this.data.goodsList.map(item => { return { GoodsId: item.goodsId, Num: item.number } }),
      TotalFee: 0,
      OfferFee: 0,
      OfferType: 0,
      ...
      OrderName: '订单 xxxx',
      CustomerName: that.data.curAddressData.name,
      CustomerAddress: that.data.curAddressData.address,
      CustomerPhone: that.data.curAddressData.phoneNumber
    }
    // var postData = {
    //   token: loginToken,
    ...
    wx.request({
      ...
      data: JSON.stringify(postData), // 设置请求的 参数
      success: (res) => {
        wx.hideLoading();
        // if (res.data.code != 0) {
        //   wx.showModal({
        ...
        // });
        // wx.redirectTo({
        //   url: '/pages/order-details/index?orderId=' + res.data.data.orderId,
        // })
        wxpay.wxpay(app, 1, res.data.data.orderId, "/pages/my/index");
      }
    })
  },
  initShippingAddress: function () {
    var that = this;
    wx.request({
      url: app.globalData.baseUrl + '/customer/GetDefaultAddress',
      data: {
        openId: app.globalData.token
      },
      success: (res) => {
  ...
  processAllPrice: function () {
    ...
    this.setData({
      allGoodsAndYunPrice: totalPrice / 100
    })
  },
